...
repository at GitHub <https://github.com/TYPO3Fluid/Fluid>`_ for example.
...
them -- and this is as easy as adding a HTML tag such as `<image ... />` to a
HTML file.
...
logical functions such as `if ... then ... else ...`. An overview of the
available ViewHelpers and how to apply them can be found at `Fluid Powered
TYPO3 <https://fluidtypo3.org/viewhelpers/>`_ and at the `TYPO3 Wiki
<https://wiki.typo3.org/Fluid>`_. Note that both sites are community-driven and
...
folder called `Resources/`. Therefore, create the directory structure as
listed below.
...
such as `Css/`, `Images/` and `JavaScript/`. All files in these folders
...
The :file:`Private/` directory with its three sub-folders `Layouts/`,
`Partials/` and `Templates/` in contrast, require some explanation.
...
but *page layouts* do not belong into the `Layout/` directory. They are
stored in the `Templates/` directory (see below). In other words, the
`Layouts/` directory should contain the global layout for the entire website
...
columns), the `Templates/` directory may contain one or multiple HTML files.
...
would be implemented in one or more HTML files inside the `Templates/`
...
store the HTML code of the box as a small HTML snippet into the `Partials/`
...
`Templates/` directories are mandatory for a typical site package extension.
...
folders :file:`Layouts/`, `Templates/` and `Partials/` all show a sub-
directory `Page/`.
...
the exact sub-directory pops up though: is `Resources/Private/Layouts/Page/`
or `Resources/Private/Templates/Page/` the perfect fit?

In our case, directory `Resources/Private/Templates/Page/` is the correct
folder, because this is the entry point for all page templates, despite the
fact that our `index.html` file in fact implements the layout of the entire
site. Therefore, the `index.html` file gets copied into
`Resources/Private/Template/Page/` and renamed to `Default.html` (in order
...
contains four files only: `Default.html`, `website.css`, `logo.png` and
`website.js`. The rest are empty directories for now.
...
(e.g. that the path should be `Resources/Private/Templates/Page/`) and the
...
As described before, a typical static `index.html` file contains a `<head>`
and a `<body>` section, but we only need to focus on the `<body>`. Open
file `site_package/Resources/Private/Templates/Page/Default.html` in your
favorite text editor and remove all lines before the starting `<body>` tag
and after the closing `</body>` tag. Then, remove these two lines, too. As a
result, file `Default.html` only contains the HTML code *inside* the body.
...
`Bootstrap Jumbotron <http://getbootstrap.com/docs/4.0/examples/jumbotron/>`_.
...
the `Resources/Private/Template/Page/Default.html` file, too.
...
partial. Create a new file named `Jumbotron.html` inside directory
`site_package/Resources/Private/Partials/Page/` and copy the approriate six
lines (starting from `<div class="jumbotron">`) into it. Make sure the file
name reads **exactly** as stated above with upper case "J" as the first
character.

Now, remove the lines from file
`Resources/Private/Template/Page/Default.html` and replace them with the
...
with `<f:...>` are typically core ViewHelpers in Fluid. The tag
...
because of the `partial="..."` argument. Note: do not append `.html` here.
HTML is the default format and as a convention, the ViewHelper automatically
knows the file name and its location: `Partials/Page/Jumbotron.html`.

At this point, we have implemented an (optional) partial and a page layout
template. Keep the file `Resources/Private/Template/Page/Default.html` open
in your text editor, because we need to make one more small adjustment.

As described above, files inside the `Templates/` directory are page-specific
...
`Resources/Private/Layouts/Page/` folder that we name `Default.html`, too.
Before we create this file, we need to tell our page layout template
(`Resource/Templates/Page/Default.html`) which website template it should
use.

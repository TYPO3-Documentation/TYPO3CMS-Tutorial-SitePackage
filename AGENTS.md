# AGENTS.md — TYPO3 Tutorial: Site Package
...
- `make test-docs` — render in fail-on-log/minimal-test mode; use this to validate any change before committing
- `make codesnippets` — regenerate `Documentation/CodeSnippets/` from the example site package
...
6. **Do not hand-edit `Documentation/CodeSnippets/my_site_package/`** — it is a
   ...
   first of every month, so a change made here disappears without a conflict and
   without a warning. Fix it in that repository instead; the next regeneration
...
- If the change should be backported, add a `Releases: main, 14.3, 13.4`
  ...
  LTS branches (currently `14.3`, `13.4`, and older unmaintained ones), so
  `Releases:` applies here.
...
- [TYPO3-Documentation/site_package](https://github.com/TYPO3-Documentation/site_package) — upstream source of `Documentation/CodeSnippets/my_site_package/`

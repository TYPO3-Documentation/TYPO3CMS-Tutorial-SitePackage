...
Documentation/CodeSnippets/my_site_package/ # GENERATED by the Site Package Builder — never edit, see rule 6
Documentation/<Chapter>/_codesnippets/      # hand-written snippets and diffs — edit these here
Build/DownloadSitePackage/                  # the download script and the data.json it sends
...
6. **Do not hand-edit `Documentation/CodeSnippets/my_site_package/`** — it is
   generated, not source of this manual. `make codesnippets` posts
   `Build/DownloadSitePackage/data.json` to the Site Package Builder at
   get.typo3.org, empties the directory and unpacks the ZIP it receives, so a
   change made here disappears without a conflict and without a warning. The
   file to change is `data.json`, which decides the package title, the TYPO3
   version and the author details. The `_codesnippets/` directories inside the
   chapters (`ContentMapping/`, `FluidTemplates/`) only look similar — they are
...
- https://get.typo3.org/sitepackage — the Site Package Builder that generates `Documentation/CodeSnippets/my_site_package/`
